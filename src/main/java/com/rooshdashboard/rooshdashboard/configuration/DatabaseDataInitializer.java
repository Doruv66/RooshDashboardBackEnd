...
public class DatabaseDataInitializer {
    ...
    private ParkingGarageUtilityRepository parkingGarageUtilityRepository;
    ...
    public void populateDatabaseInitialDummyData() {
        ...
        ParkingGarageEntity savedParkingGarage = ParkingGarageEntity.builder().location("123 Main St").airport("s").name("s").phoneNumber("123").travelDistance(12L).travelTime(12L).account(savedAccount).build();
        if (bookingRepository.count() == 0) {
            customerRepository.save(savedCustomer);
            carRepository.save(savedCar);
            parkingGarageRepository.save(savedParkingGarage);
            parkingGarageUtilityRepository.save(savedParkingGarageUtility);
